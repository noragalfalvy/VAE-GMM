...
# %%
# Data preprocessing (PRS-specific loading)
def load_and_preprocess_prs_data(file_path, phenotype=None, train_size=0.6, val_size=0.2, test_size=0.2):
    df = pd.read_csv(file_path, sep=r'\s+')
    
    if phenotype is not None and 'pheno' in df.columns:
        df = df[df['pheno'] == phenotype]
        print(f"Filtered to phenotype {phenotype}, remaining samples: {len(df)}")
    
    id_cols = ['FID', 'IID']
    if 'pheno' in df.columns:
        id_cols.append('pheno')
...
class BetaScheduler(keras.callbacks.Callback):
    def __init__(self, beta_var, beta_start=0.0, beta_end=1.0, annealing_epochs=100, warmup_epochs=10, verbose=0):
...
# %%
# VAE model creation (GMM prior)
def create_vae_gmm(input_dim=20, latent_dim=2, hidden_units=[64, 32], 
                  num_components=5, beta_start=0.0, l2_reg=0.001, dropout_rate=0.2, learning_rate=0.001):
    ...
    if latent_dim == 2:
        ...
        for i in range(num_components):
            ...
            mean = mean / np.sqrt(np.sum(mean**2)) * 2.0  # adjust normalization
...
def visualize_latent_space_multiple(z_mean, labels, n_components, entropies=None, gmm=None, probs=None,
    ...
    axes[0].set_title(f'Latent Space with {n_components} GMM Clusters {title_suffix}')
    ...
    if entropies is not None:
        ...
        axes[1].set_title(f'Clustering Uncertainty (Entropy) {title_suffix}')
        ...
        try:
            ...
            axes[1].set_title(f'Density Contours with Data Points {title_suffix}')
            axes[1].set_xlabel('Latent Dimension 1')
            axes[1].set_ylabel('Latent Dimension 2')
            axes[1].grid(alpha=0.3)
        except:
            axes[1].scatter(z_mean[:, 0], z_mean[:, 1], c=labels, cmap=cmap, 
                         alpha=0.8, s=25)
            axes[1].set_title(f'Latent Space (Alternative View) {title_suffix}')
            axes[1].set_xlabel('Latent Dimension 1')
            axes[1].set_ylabel('Latent Dimension 2')
            axes[1].grid(alpha=0.3)
...
def plot_loss_components(history):
    ...
    plt.subplot(1, 3, 2)
    plt.plot(history['metric_reconstruction_loss'], label='Train Recon', color='blue')
    plt.plot(history['val_metric_reconstruction_loss'], label='Val Recon', color='red')
    ...
    plt.subplot(1, 3, 3)
    plt.plot(history['metric_kl_loss'], label='KL Div', color='purple')
    if 'val_metric_kl_loss' in history:
        plt.plot(history['val_metric_kl_loss'], label='Val KL Div', color='orange')
...
# %%
# Main pipeline (PRS-specific with anti-overfitting)
def run_prs_vae_pipeline(file_path, phenotype=None, latent_dim=2, hidden_units=[32, 16], 
                         min_clusters=2, max_clusters=8, batch_size=32, epochs=100,
                         beta_start=0.0, beta_end=0.5, l2_reg=0.005, dropout_rate=0.4,
                         learning_rate=0.0005, warmup_epochs=8, hold_base_epochs=5,
                         annealing_epochs=50, min_val_patience=5, early_stop_patience=10,
                         show_plots=True, save_dir='./prs_vae_results'):
    
    phenotype_name = "all"
    if phenotype == 1:
        phenotype_name = "controls"
    elif phenotype == 2:
        phenotype_name = "cases"
    
    full_save_dir = os.path.join(save_dir, phenotype_name)
    ...
    config = {
        ...
        'hold_base_epochs': hold_base_epochs,
        'annealing_epochs': annealing_epochs
    }
    
    with open(f"{full_save_dir}/config.txt", 'w') as f:
        for key, value in config.items():
            f.write(f"{key}: {value}\n")
    
    print(f"Loading and preprocessing PRS data for {phenotype_name}...")
    X_train, X_val, X_test, X_scaled, scaler, ids, y, feature_cols = load_and_preprocess_prs_data(
        file_path, phenotype=phenotype)
    ...
    if show_plots:
        plt.savefig(f"{full_save_dir}/initial_cluster_estimate.png", dpi=300, bbox_inches='tight')
        plt.show()
    else:
        plt.close()
    ...
    early_stopping = tf.keras.callbacks.EarlyStopping(
        ...
        min_delta=0.005,  # adjust minimum improvement threshold
        restore_best_weights=True,
        verbose=1
    )
    
    checkpoint = BestModelCheckpoint(
        f"{full_save_dir}/best_vae_model.h5",
        monitor='val_loss',
    ...
    # Minimum validation loss callback (anti-overfitting)
    class MinValLossCallback(tf.keras.callbacks.Callback):
        def __init__(self, patience=5):
            super(MinValLossCallback, self).__init__()
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if current_loss < self.best_loss:
                ...
                if self.wait >= self.patience:
                    print(f"\nReached minimum validation loss at epoch {self.best_epoch+1}")
                    print(f"Stopping training and restoring best weights")
                    self.model.stop_training = True
                    self.model.set_weights(self.best_weights)
    
    min_val_loss_callback = MinValLossCallback(patience=min_val_patience)
    
    print(f"Training VAE for {phenotype_name}...")
    history = vae.fit(
        X_train, X_train,
        epochs=epochs,
        batch_size=batch_size,
        validation_data=(X_val, X_val),
        callbacks=[beta_scheduler, early_stopping, checkpoint, lr_scheduler, min_val_loss_callback],
        verbose=1
    )
    ...
    with open(f"{full_save_dir}/overfitting_metrics.txt", 'w') as f:
    ...
    if show_plots:
        plt.savefig(f"{full_save_dir}/training_loss.png", dpi=300, bbox_inches='tight')
        plt.show()
    else:
        plt.close()
    ...
    if show_plots:
        plt.savefig(f"{full_save_dir}/optimal_clusters.png", dpi=300, bbox_inches='tight')
        plt.show()
    else:
        plt.close()
    ...
    with open(f"{full_save_dir}/cluster_metrics.txt", 'w') as f:
    ...
    metrics_df.to_csv(f"{full_save_dir}/cluster_metrics_comparison.csv", index=False)
    ...
    if optimal_clusters >= 2:
        ...
        with open(f"{full_save_dir}/stability_metrics.txt", 'w') as f:
        ...
        plt.savefig(f"{full_save_dir}/cluster_stability.png", dpi=300, bbox_inches='tight')
    ...
    latent_plot = visualize_latent_space_multiple(
        z_latent, labels, optimal_clusters, entropies, gmm, probs,
        title_suffix=f" ({phenotype_name})"
    )
    if show_plots:
        plt.savefig(f"{full_save_dir}/latent_space.png", dpi=300, bbox_inches='tight')
        plt.show()
    else:
        plt.close()
    ...
    result_df.to_csv(f"{full_save_dir}/cluster_assignments.csv", index=False)
    
    print(f"\nFound {optimal_clusters} clusters in the {phenotype_name} data")
...
# %%
# Analysis for cases and controls
def analyze_cases_and_controls(prs_file_path, save_dir='./prs_vae_results'):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    
    print("\n" + "="*80)
    print(f"ANALYZING CASES (phenotype=2) FROM: {prs_file_path}")
    print("="*80 + "\n")
    
    cases_results = run_prs_vae_pipeline(
        file_path=prs_file_path,
        phenotype=2,  # Cases
        latent_dim=2,  # adjust dimensions
        hidden_units=[64, 32],  # adjust architecture
        min_clusters=2,  # adjust minimum clusters
        max_clusters=8,  # adjust maximum clusters
        batch_size=32,  # adjust batch size
        epochs=100,  # adjust epochs
        beta_start=0.0,  # adjust KL weight start
        beta_end=1.0,  # adjust KL weight end
    ...
    print("\n" + "="*80)
    print(f"ANALYZING CONTROLS (phenotype=1) FROM: {prs_file_path}")
    print("="*80 + "\n")
    
    controls_results = run_prs_vae_pipeline(
        file_path=prs_file_path,
    ...
    compare_cases_and_controls(cases_results, controls_results, save_dir)
    
    return cases_results, controls_results

# %%
# Comparison between cases and controls
def compare_cases_and_controls(cases_results, controls_results, save_dir):
    comparison_dir = os.path.join(save_dir, "comparison")
    if not os.path.exists(comparison_dir):
        os.makedirs(comparison_dir)
    
    print("\n" + "="*80)
    print("COMPARING CASES AND CONTROLS CLUSTERING RESULTS")
    print("="*80 + "\n")
    
    cases_df = cases_results['result_df']
    controls_df = controls_results['result_df']
    cases_z = cases_results['z_latent']
    controls_z = controls_results['z_latent']
    cases_labels = cases_results['labels']
    controls_labels = controls_results['labels']
    cases_clusters = cases_results['optimal_clusters']
    controls_clusters = controls_results['optimal_clusters']
    
    print(f"Number of clusters found in cases: {cases_clusters}")
    print(f"Number of clusters found in controls: {controls_clusters}")
    
    print("\nCluster distribution for cases:")
    cases_cluster_counts = cases_df['Cluster'].value_counts().sort_index()
    for cluster, count in cases_cluster_counts.items():
        percentage = count/len(cases_df)*100
        print(f"  Cluster {cluster}: {count} samples ({percentage:.1f}%)")
    
    print("\nCluster distribution for controls:")
    controls_cluster_counts = controls_df['Cluster'].value_counts().sort_index()
    for cluster, count in controls_cluster_counts.items():
        percentage = count/len(controls_df)*100
        print(f"  Cluster {cluster}: {count} samples ({percentage:.1f}%)")
    ...
    plt.title(f'Cases Latent Space ({cases_clusters} clusters)')
    plt.xlabel('Latent Dimension 1')
    plt.ylabel('Latent Dimension 2')
    plt.grid(alpha=0.3)
    ...
    plt.title(f'Controls Latent Space ({controls_clusters} clusters)')
    plt.xlabel('Latent Dimension 1')
    plt.ylabel('Latent Dimension 2')
    plt.grid(alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f"{comparison_dir}/latent_space_comparison.png", dpi=300, bbox_inches='tight')
    plt.show()
    
    # Cluster metrics comparison
    if isinstance(cases_results['cluster_metrics'], dict) and isinstance(controls_results['cluster_metrics'], dict):
        print("\nClustering quality metrics comparison:")
        
        metrics_table = []
        
        if 'latent' in cases_results['cluster_metrics'] and 'latent' in controls_results['cluster_metrics']:
            cases_latent_metrics = cases_results['cluster_metrics']['latent']
            controls_latent_metrics = controls_results['cluster_metrics']['latent']
            
            metrics_table.append(['Silhouette Score (Latent)', 
                                 cases_latent_metrics['silhouette'], 
                                 controls_latent_metrics['silhouette']])
            
            metrics_table.append(['Davies-Bouldin Index (Latent)', 
                                 cases_latent_metrics['davies_bouldin'], 
                                 controls_latent_metrics['davies_bouldin']])
            
            metrics_table.append(['Calinski-Harabasz Index (Latent)', 
                                 cases_latent_metrics['calinski_harabasz'], 
                                 controls_latent_metrics['calinski_harabasz']])
        else:
            metrics_table.append(['Silhouette Score', 
                                 cases_results['cluster_metrics'].get('silhouette', 'N/A'), 
                                 controls_results['cluster_metrics'].get('silhouette', 'N/A')])
            
            metrics_table.append(['Davies-Bouldin Index', 
                                 cases_results['cluster_metrics'].get('davies_bouldin', 'N/A'), 
                                 controls_results['cluster_metrics'].get('davies_bouldin', 'N/A')])
            
            metrics_table.append(['Calinski-Harabasz Index', 
                                 cases_results['cluster_metrics'].get('calinski_harabasz', 'N/A'),
                                 controls_results['cluster_metrics'].get('calinski_harabasz', 'N/A')])
        
        print("\nMetric                     | Cases        | Controls")
        print("-----------------------------------------------------------")
        for row in metrics_table:
            print(f"{row[0]:<28} | {row[1]:<13.4f} | {row[2]:<13.4f}")
    
    # Stability comparison
    if cases_results['stability_results'] is not None and controls_results['stability_results'] is not None:
        print("\nCluster stability comparison:")
        print(f"Mean Adjusted Rand Index (Cases): {cases_results['stability_results']['mean_adjusted_rand_index']:.4f}")
        print(f"Mean Adjusted Rand Index (Controls): {controls_results['stability_results']['mean_adjusted_rand_index']:.4f}")
        
        print(f"\nMean Adjusted Mutual Information (Cases): {cases_results['stability_results']['mean_adjusted_mutual_info']:.4f}")
        print(f"Mean Adjusted Mutual Information (Controls): {controls_results['stability_results']['mean_adjusted_mutual_info']:.4f}")
    
    # Feature importance analysis
    print("\nAnalyzing feature importance in latent space encoding...")
    
    cases_weights = cases_results['encoder'].layers[1].get_weights()[0]
    controls_weights = controls_results['encoder'].layers[1].get_weights()[0]
    
    cases_importance = np.mean(np.abs(cases_weights), axis=1)
    controls_importance = np.mean(np.abs(controls_weights), axis=1)
    
    feature_names = cases_results['result_df'].columns.drop(['FID', 'IID', 'pheno', 'Cluster', 
                                                            'LatentDim1', 'LatentDim2', 
                                                            'Uncertainty'] + 
                                                           [col for col in cases_results['result_df'].columns 
                                                            if col.startswith('Prob_Cluster_') or 
                                                            col in ['Sample_Stability', 'Cluster_Entropy']])
    
    if len(feature_names) != len(cases_importance):
        feature_names = [f"Feature_{i}" for i in range(len(cases_importance))]
    
    importance_df = pd.DataFrame({
        'Feature': feature_names,
        'Cases_Importance': cases_importance,
        'Controls_Importance': controls_importance,
        'Difference': np.abs(cases_importance - controls_importance)
    })
    
    importance_df = importance_df.sort_values('Difference', ascending=False)
    
    importance_df.to_csv(f"{comparison_dir}/feature_importance_comparison.csv", index=False)
    
    # Feature importance visualization
    plt.figure(figsize=(14, 10))
    top_n = min(15, len(importance_df))  # adjust number of features to display
    top_features = importance_df.head(top_n)
    
    ind = np.arange(top_n)
    width = 0.35  # adjust bar width
    
    plt.barh(ind, top_features['Cases_Importance'], width, label='Cases', color='#1f77b4', alpha=0.8)
    plt.barh(ind + width, top_features['Controls_Importance'], width, label='Controls', color='#ff7f0e', alpha=0.8)
    
    plt.yticks(ind + width/2, top_features['Feature'])
    plt.xlabel('Feature Importance (Mean Absolute Weight)')
    plt.title('Top Differentially Important Features Between Cases and Controls')
    plt.legend(loc='best')
    ...
    print(f"\nFeature importance analysis saved to {comparison_dir}/feature_importance_comparison.csv")
    print(f"Top {top_n} differentially important features visualization saved.")
    
    # Save summary
    with open(f"{comparison_dir}/comparison_summary.txt", 'w') as f:
        f.write("COMPARISON BETWEEN CASES AND CONTROLS CLUSTERING\n")
        f.write("="*50 + "\n\n")
        
        f.write(f"Number of clusters found in cases: {cases_clusters}\n")
        f.write(f"Number of clusters found in controls: {controls_clusters}\n\n")
        
        f.write("Cluster distribution for cases:\n")
        for cluster, count in cases_cluster_counts.items():
            percentage = count/len(cases_df)*100
        ...
        f.write("\nClustering quality metrics comparison:\n")
        f.write("Metric                     | Cases        | Controls\n")
        f.write("-----------------------------------------------------------\n")
        for row in metrics_table:
            f.write(f"{row[0]:<28} | {row[1]:<13.4f} | {row[2]:<13.4f}\n")
        
        if cases_results['stability_results'] is not None and controls_results['stability_results'] is not None:
            f.write("\nCluster stability comparison:\n")
            f.write(f"Mean Adjusted Rand Index (Cases): {cases_results['stability_results']['mean_adjusted_rand_index']:.4f}\n")
            f.write(f"Mean Adjusted Rand Index (Controls): {controls_results['stability_results']['mean_adjusted_rand_index']:.4f}\n")
            
            f.write(f"\nMean Adjusted Mutual Information (Cases): {cases_results['stability_results']['mean_adjusted_mutual_info']:.4f}\n")
            f.write(f"Mean Adjusted Mutual Information (Controls): {controls_results['stability_results']['mean_adjusted_mutual_info']:.4f}\n")
        
        f.write("\nTop 10 differentially important features:\n")
        for i, (_, row) in enumerate(top_features.head(10).iterrows()):
            f.write(f"{i+1}. {row['Feature']}: Cases={row['Cases_Importance']:.4f}, Controls={row['Controls_Importance']:.4f}, Diff={row['Difference']:.4f}\n")
...
if __name__ == "__main__":
    # Configuration
    prs_file_path = "scaled_cases.txt"  # replace with your data file
    output_dir = "autismsubs/people/gnora/VAE_PRS_cases"  # adjust output directory
    mode = "cases"  # "cases", "controls", or "both"
    
    # Anti-overfitting configuration (adjust all values as needed)
    latent_dim = 2
    hidden_units = [24, 12]  # adjust architecture
    learning_rate = 0.0003  # adjust learning rate
    batch_size = 64  # adjust batch size
    epochs = 100  # adjust max epochs
    
    # Learning rate scheduler parameters
    warmup_epochs = 15  # adjust warmup period
    hold_base_epochs = 20  # adjust hold period
    
    # Regularization parameters 
    beta_start = 0.0  # adjust KL weight start
    beta_end = 1.8  # adjust KL weight end
    annealing_epochs = 80  # adjust annealing period
    l2_reg = 0.01  # adjust L2 regularization
    dropout_rate = 0.25  # adjust dropout rate
    
    # Early stopping parameters
    min_val_patience = 5  # adjust minimum validation patience
    early_stop_patience = 25  # adjust early stopping patience
    
    # Clustering parameters
    min_clusters = 1  # adjust minimum clusters
    max_clusters = 6  # adjust maximum clusters
    
    # Run analysis
    run_prs_vae_pipeline(
        file_path=prs_file_path,
        phenotype=2,  # Cases:2, Controls:1
        latent_dim=latent_dim,
        hidden_units=hidden_units,
        learning_rate=learning_rate,
        min_clusters=min_clusters,
        ...
        warmup_epochs=warmup_epochs,
        hold_base_epochs=hold_base_epochs,
        min_val_patience=min_val_patience,
